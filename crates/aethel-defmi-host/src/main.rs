...
async fn main() -> ExitCode {
    match run().await {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("aethel-defmi-host: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn genesis_command(arguments: &[String]) -> Result<(), String> {
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            _ => {
                return Err("usage: aethel-defmi-host genesis --config FILE [--out FILE|-]".into())
            }
        }
    }
    let config = config.ok_or_else(|| {
        "usage: aethel-defmi-host genesis --config FILE [--out FILE|-]".to_string()
    })?;
